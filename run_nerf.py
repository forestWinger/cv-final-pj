...
def train():
        ...
        with torch.no_grad():
            ...
            if images is not None:
                ...
                for i in range(len(images)):
                    psnr = mse2psnr(img2mse(torch.Tensor(rgbs[i]), torch.Tensor(images[i])))
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_img == 0:
            ...
            loss2 = img2mse(rgb, target)
            psnr = mse2psnr(loss2)

            writer.add_scalar('psnr_holdout', psnr, global_step=i)
            writer.add_scalar('test_loss', loss.item(), global_step=i)  # 将测试集损失记录到 TensorBoard
